...
export function renderEmail(payload) {
  ...
  const fxText = fx
    ? [
        ...
        ...fx.pairs.map((row) => `${row.pair}: ${row.rate} (${row.change_percent})`),
      ]
    : [];
  const fxHtml = fx
    ? `<hr><h2>Foreign Exchange — ${escapeHtml(fx.label)}</h2>
    <p><strong>Observation date:</strong> ${escapeHtml(fx.observation_date)}</p>
    ${fx.pairs.map((row) => `<p><strong>${escapeHtml(row.pair)}:</strong> ${escapeHtml(row.rate)} (${escapeHtml(row.change_percent)})</p>`).join("\n")}`
    : "";
