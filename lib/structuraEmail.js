...
export const OPTIONAL_INDEXES = new Set(["Straits Times Index"]);
...
export function validateStructuraPayload(payload) {
  ...
  if (typeof brief.x_ready_post === "string") {
    ...
    if (actualCount > 280) errors.push("brief.x_ready_post exceeds 280 characters");
  ...
  if (!isObject(brief.overseas)) {
    ...
    if (!Array.isArray(brief.overseas.indexes)) {
      ...
      const stiCount = names.filter((item) => item === "Straits Times Index").length;
      if (stiCount > 1) {
        errors.push("brief.overseas.indexes may contain Straits Times Index at most once");
      }
      const allowedNames = new Set([...REQUIRED_INDEXES, ...OPTIONAL_INDEXES]);
      for (const name of names) {
        if (!allowedNames.has(name)) {
          errors.push(`brief.overseas.indexes contains an unexpected index: ${String(name)}`);
        }
      }
      const expectedLength = REQUIRED_INDEXES.length + (stiCount === 1 ? 1 : 0);
      if (names.length !== expectedLength) {
        errors.push("brief.overseas.indexes must contain the required six indexes with optional Straits Times Index");
      }
...
export function renderEmail(payload) {
  ...
  const asia = brief.overseas.indexes
    .filter((row) => ["ASX 200", "Nikkei 225", "Straits Times Index"].includes(row.name))
    .map((row) => `${row.name} ${row.change_percent}`)
    .join(", ");
  const europe = brief.overseas.indexes
    .filter((row) => ["FTSE 100", "DAX", "CAC 40", "EURO STOXX 50"].includes(row.name))
    .map((row) => `${row.name} ${row.change_percent}`)
    .join(", ");
